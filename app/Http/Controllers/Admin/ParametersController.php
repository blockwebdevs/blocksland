...
class ParametersController extends Controller
{
    ...
    {
        $names = array_filter($request->get('name'), function ($var) {
            return !is_null($var);
        });
        $units = array_filter($request->get('unit'), function ($var) {
            return !is_null($var);
        });
        $languages = array_filter($request->get('langs'), function ($var) {
            return !is_null($var);
        });
...
//        $this->attributeParameterTocategory($request, $parameter);
    ...
    {
        $categoriesId = array_filter($request->get('categories'), function ($var) {
            return !is_null($var);
        });
        $categoriesId = array_filter($categoriesId, function ($var) {
            return $var !== false;
        });
    ...
    private function saveNewPrameterValues($request, $parameter)
    {
        $paramValues = [];
        $paramSuffixes = [];
        if ($request->get('propSuffix')) {
            $paramSuffixes = array_filter($request->get('propSuffix'), function ($var) {
                return !is_null($var);
            });
        }
        ...
        if (count($request->get('paramValues'))) {
            foreach ($request->get('paramValues') as $key => $value) {
                $paramValues[] = array_filter($value, function ($var) {
                    return !is_null($var);
                });
            }
        }
    ...
    {
        $paramValues = [];
        $paramSuffixes = array_filter($request->get('oldPropSuffix'), function ($var) {
            return !is_null($var);
        });
        ...
        if (count($request->get('oldParamValues'))) {
            foreach ($request->get('oldParamValues') as $key => $value) {
                if (is_array($value)) {
                    $paramValues[$key] = array_filter($value, function ($var) {
                        return !is_null($var);
                    });
                }
            }
    ...
    public function edit($id)
    {
        ...
        return view(' admin.parameters.edit', compact('parameter', 'categories', 'values'));
    }
