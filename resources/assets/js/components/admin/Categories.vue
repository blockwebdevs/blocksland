            ...
            <vue-nestable-handle slot-scope="{ item }" :item="item">
                <div class="row">
                    ...
                    <div class="col-md-5 text-right">
                        <a :href="'/back/products/category/' + item.id"
                           v-if="item.products.length || !item.children.length"><i class="fa fa-eye"></i></a>

<!--                        <a href="#" data-toggle="modal" @click="setCurrentCategory(item)" data-target="#addNew"-->
<!--                           v-if="!item.products.length && item.level < maxDepth"><i class="fa fa-plus"></i></a>-->
<!--                        <a href="#" v-else><i class="fa fa-minus"></i></a>-->
        ...
        <!-- Add new Modal -->
        <div v-if="currentCategory" class="modal fade settings-modal" id="addNew" tabindex="-1" role="dialog"
            ...
            <div class="modal-dialog modal-md" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <h6 class="modal-title">Add new in
                            "<b>{{ currentCategory.translation ? currentCategory.translation.name : '' }}</b>"</h6>
                    </div>
                        ...
                        <div class="col-md-12">
                            <div class="col-md-6">
                                <button type="button" class="btn btn-primary btn-block" data-dismiss="modal"
                                        aria-label="Close" @click="addSave">Add & Close
                                </button>
                            </div>
                            <div class="col-md-6">
                                <button type="button" class="btn btn-primary btn-block" @click="addEdit">Add & Edit
                                </button>
                            </div>
                        </div>
                    </div>
        ...
        <!-- Remove Allert Modal -->
        <div v-if="currentCategory" class="modal fade settings-modal" id="allertRemove" tabindex="-1" role="dialog"
             aria-labelledby="myModalLabel">
            <div class="modal-dialog modal-md" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                            aria-hidden="true">&times;</span></button>
                        <h6 class="modal-title">Warning removing
                            "<b>{{ currentCategory.translation ? currentCategory.translation.name : '' }}</b>"</h6>
                    </div>
                    <div class="modal-body" v-if="Object.keys(currentCategory).length">
                        <div class="row" v-if="currentCategory.products.length > 0">
                            <p class="text-center">
                                "<b>{{ currentCategory.translation ? currentCategory.translation.name : '' }}</b>"
                                contains items, you want to move them to another category?</p>
                            <div class="col-md-12">
                                <div class="form-group">
                                    <select class="form-control" v-model="categoryToMove">
                                        <option value="0">---</option>
                                        <option :value="category.id" v-for="category in allCategories"
                                                v-if="!category.children.length && category.id !== currentCategory.id">
                        ...
                        <div class="row" v-if="currentCategory.children.length > 0">
                            <p class="text-center text-danger">
                                "<b>{{ currentCategory.translation ? currentCategory.translation.name : '' }}</b>"
                                contains categories, you want to move them to another category?</p>
                            <div class="col-md-12">
                                <div class="form-group">
                                    <select class="form-control" v-model="categoryToMove">
                                        <option value="0">---</option>
                                        <option :value="category.id" v-for="category in allCategories"
                                                v-if="!category.products.length && category.level < maxDepth && category.id !== currentCategory.id">
                                            {{ category.translation.name }}
                                        </option>
                                    </select>
                                </div>
        ...
        <div v-if="currentCategory" class="modal fade settings-modal" id="allertValiadate" tabindex="-1" role="dialog"
             aria-labelledby="myModalLabel">
            <div class="modal-dialog modal-md" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                            aria-hidden="true">&times;</span></button>
                        <h6 class="modal-title">Warning,
                            "<b>{{ errorParent.translation ? errorParent.translation.name : '' }}</b>" contain products
                        </h6>
                    </div>
                    <div class="modal-body text-center">
                        <p class="text-center">
                            Do you want to move the products to
                            "<b>{{ errorChild.translation ? errorChild.translation.name : '' }}</b>"
                        </p>
                    </div>
                    <div class="row">
                        <div class="col-md-12 text-center">
                            <button type="button" class="btn btn-primary" data-dismiss="modal" aria-label="Close"
                                    @click="moveProducts">Da
                            </button>
                            <button type="button" class="btn btn-primary" data-dismiss="modal" aria-label="Close">Nu
                            </button>
                        </div>
                    </div>
                    <hr>
    ...
    methods: {
        load() {
            axios.post('/back/product-categories/get-categories', {})
                ...
                .catch(e => {
                    console.log('error load ctegories')
                });
        },
        setTitles() {
            let ret = [];
                            ...
                            cat2.children.forEach(function (cat3) {
                                ...
                                if (cat3.children.length > 0) {
                                    cat3.children.forEach(function (cat4) {
                                        if (cat3.products.length > 0) return ret = {
                                            status: false,
                                            parent: cat3,
                                            child: cat4
                                        };
                                        ...
                                        if (cat4.children.length > 0) {
                                            cat4.children.forEach(function (cat5) {
                                                ...
                                                if (cat4.products.length > 0) return ret = {
                                                    status: false,
                                                    parent: cat4,
                                                    child: cat5
                                                };
        ...
        moveProducts() {
            axios.post('/back/product-categories/move-products', {
                parent_category: this.errorParent.id,
                child_category: this.errorChild.id
            })
                .then(response => {
                    this.categories = response.data;
                })
                .catch(e => {
                    console.log('error remove categories')
                });
        },
        ...
        remove(category) {
            ...
            axios.post('/back/product-categories/get-all-categories', {category_id: category.id})
                ...
                .catch(e => {
                    console.log('error load all categories')
                });
        },
        moveRevome() {
            axios.post('/back/product-categories/remove-moving-category', {
                category_to_move: this.categoryToMove,
        ...
        addSave() {
            axios.post('/back/product-categories/add-new', {
                titles: this.titles,
